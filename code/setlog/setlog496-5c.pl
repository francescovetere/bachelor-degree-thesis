...
% ATTN. non funziona  S is ris(X in {a,b/R},true). - DA SISTEMARE

% ATTN. finire di sistemare casi di occur-check in vincoli con RIS e trattamento
% di vincoli su RIS non ammissibili (RIS infiniti)

%ATTN la neq su RIS e CP quando uno degli argomenti e' variabile assume che sia
%di tipo set; quindi ad es. a neq ris(...) oppure a neq cp(...) rispondono 'no'
%invece a neq int(1,10) ripsonde 'yes'; analogamente, X neq int(1,10) viene 
%considerato in solved form, mentre X neq ris(...) e X neq cp(...) vengono risolti
%(in modo insiemistico)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  VERSION 4.9.6

%release 4.9.6-0
% - ripristinato C = [set(N)|C3]     
%   invece di C = [X nin N,set(N)|C3] in stunify_samevar
% - aggiunti commenti vari in parte su parametri di configurazione
% - aggiunti aggrs(...) e comp_constraint(...) in parte su par. di configuarazione (per ora commentati)

%release 4.9.6-1
% - modificata clausola di sat_un per trattamento caso un(s1,s2,{...})  
%   (aggiunto final_sat([N with T1=S3,T1 nin N],C1));
%   controesempi: un(X,{a/Y},{b/Z}) & un(X,V,Z), un(X,{a/Y},Z) & un(X,V,Z)
% - aggiunto setlog5_opts/1 in parte su par. di configuarazione e usato in setlog/4 
% - tolto cond_retract(final) in top_level; controesempio: il goal
%   dom(R,D) & dom(S,D) & D neq {} & R neq {} & S neq {} &  disj(S,R) & un(S,R,T) &  pfun(T).
%   funziona solo con 'final' e con il cond_retract(final) in top_level non attiva mai 'final'
%a
% - eliminato bug in sat_un_v su controllo occur-check (spostato cut in penultima clausola);
%   controesempi: un({a,X/R},{b/S},X), un({a,X/R},{b},X), un({a/R},{b,X},X) (prima andavano in loop)
%b
% - aggiunto occur_check(on/off) in parte su par. di configuarazione (per ora commentato)
% - problema con goal un({X},cp(B,C),cp(A,A)): viene trattato dal caso unCP - un({.../S},cp(A,A),cp(A,A))
%   modificato trattamento caso un({.../S},cp(A,A),cp(A,A)) in sat_un_vcp
%   ma cosi' il goal un({X},cp(B,C),cp(A,A)) va in ciclo dopo due soluzioni
%   inoltre, loop con goal  {X/cp(B,C)} = cp(A,A) dopo due soluzioni!!! (anche con versioni precedenti;
%   (nella versione su web (setlog495-22 x web) da' piu' soluzioni e poi dopo un po' si ferma)
%   ripristinata definizione della penultima clausola di cp_to_set_u(N,X,U) che utilizza
%   unificazione insiemistica tramite la get_elem/4   
%   (N.B. funzionerebbe piu' velocemente senza la final_sat in  sat_un)
%c
% - sostituito tail con tail_cp in sat_sub special cases
%   controesempio: subset(A,cp(A,A)) & A neq {}
%d
% - aggiunto chiamate a set_member_strong in sat_in_s e in sat_nin
%   esempi:  X in {1,X,2}, X nin {1,X,2}
%e
% - aggiunta possibilita' di disattivare occur-check (occur_check(on/off))
% - aggiunta possibilita' di trattare solo vincoli su insiemi (other_aggrs(on/off));
%   modifiche in: sat_neq_nn, sat_in_v, global_check2 (liste), aggr_comps

%release 4.9.6-2
%a
% - sistemati bug vari nella sat_riseq (= espansione del RIS)
%   controesempi:
%   X is ris(Z in D,Z>0 & Z in X).  ===> loop
%   X={2,3} & X is ris(Z in {2,3},Z>0 & Z in X).  ===> no (con X={3,2}, yes)
%   X is ris(Z in {2,3},Z>0 & Z in X).   ===> manca la soiluzione X={2,3}
%   X=int(3,2) & X is ris(Z in {2,3},Z>4 & Z in X). ===> no (con X={}, yes)
% - aggiunto cut nell'ultima clausola di sat_riseq
%   controesempio: X is ris(Z in {2,3},Z>4 & Z in X) (4 sol. identiche X={})
% - definito e usato dunify in sat_riseq
%   controesempio:  X is ris(Z in {2,3},Z>0 & Z in X). (tutte sol. doppie)
%b
% - aggiunto predicato var_or_ground_elems e modificato (e spostato) definizione
%   di ground_elems
% - aggiunto predicato get_rest/4 e modificato (e spostato) definizione 
%   di first_rest; sostituite chiamate (non tutte) di first_rest con get_rest
% - spostata definizione di negate/2
% - spostate (e commentati) find_corr_list, chvar/7 e chvar_all/7, is_cp, same_tail,
%   unify_empty, vari predicati che operano su CP (messi in auxiliary pred's for CP)
% - esteso il predicato extract_vars per trattare anche il caso di ris
%c
% - implementato disj sui RIS: aggiunto caso a sat_disj; definito predicato sat_disj_ris
%d
% - corretto bug in ctrl_expr (uso cut)
% - aggiunta clausola di cp_preproc per stampa errore
% - aggiunto msg_sort_error(cp) e msg_sort_error(ris)
% - usato int_term/1 invece di is_int in is_set(S) (cambiato is_set in aggr_term)
% - usato ris_term/1 invece di is_ris in is_set(S) (cambiato is_set in aggr_term)
% - aggiunte definizioni di ris_preproc/3, check_ris/1, ctrl_expr_ok/1 
% - spostata definizione di is_ris/2 in parte preprocessor
% - aggiunto chiamata a ris_preproc/3 in preproc/3  
% - sostituito uso is_cp con cp_term/1 o cp_term/3
% - sostituito nonvar(T),is_set(T) con aggr_term(T)
% - sostituito nonvar(Y), is_ris(Y,Ris) con ris_term(Y,Ris)
% - sostituito check_ris con expandable_ris(Ris)
% - aggiunto nonvar(T1) in varie clausole di sat_eq_cp
% - aggiunto nonvar(I) in clausola di sat_nin_cp
% - aggiunto nonvar(X) in varie alternative di sat_un_vcp
% - aggiunto nonvar(X) in clausola di sat_nset([nset(X)|R1],R2,c,F) 
% - usato int_term, is_ris_ext, cp_term in is_rest_ext
% - spostato is_int(T) in parte preprocessor
% - aggiunta definizione di is_ris_ext
% - aggiunta definizione di with_term/1 (non usata)

%release 4.9.6-3
%a
% - modificata clausola di sunifyXt/3 per trattamento casi X = {...|X} e X = {...|ris(Z in X,...,Z)}:
%   definito nuovo predicato sunifyXt_special
% - scambiato ordine test var(T2) e ris_term(T1,...) in alcune clausole di sat_eq_ris
%b
% - implementato 'un' sui RIS: aggiunto caso a sat_un; definito predicato sat_un_ris
% - modificato un(Y,X,Z) in un(X,Y,Z) in prima clausola di sat_un_cp (perche' inutile girare i parametri)
% - spostata posizione 'cut' in clausole per sat_un_cp (messi prima di chiamata a cp_to_set)
% - modificato cp_to_set_t e cp_to_set_u mettendo il risultato direttamente nella testa
% - sostituito is_ris con ris_term in sat_disj
% - sostituito is_ris con ris_term in ground_elems
% - definito predicati ris_to_set_t e ris_to_set_k
%c
% - implementato 'subset' sui RIS
% - definito predicato var_ris_st e usato in sat_sub
% - modificato sat_sub in modo che anche subset(cp(A,B),s) venga trattato come irriducibile
% - modificato sat_sub in modo che subset(cp(A,B),X) venga trattato come irriducibile (invece
%   che riscriverlo in un(cp(A,B),X,X).
% - modificato sat_sub in modo che subset(cp(A,B),{}) venga trattato subito
%d
% - estesa unify_empty in modo da trattare anche il caso di RIS
% - sistemato utilizzo unify_empty in sat_un per il caso un(A,B,{})

%%release 4.9.6-4
% - sistemato switch in sat_neq; definito nuovi predicati: sat_neq_var, sat_neq_intv, 
%   sat_neq_var, sat_neq_ris_cont
%a
% - aggiunto trattamento caso speciale neq tra pair (predicato sat_neq_pair)
%b
% - sostituite chiamate a sat_step con chiamate a sat_c, con c constraint, in
%   sat_neq, sat_in, sat_nin
%c
% - aggiunto caso t1 neq t1 a sat_neq
%d
% - sostituite chiamate a sat_step con chiamate a sat_c, con c constraint, in sat_un

%%release 4.9.6-5
% - sistemato eliminazione neq per tutti i vincoli sia su CP che su RIS
%   (definito nuovo predicato var_ris_st/2)
%   controesempio:  subset(cp(A,B),Y) & A neq {} rimane in solved form
%a
% - sistemato commenti in sat_un caso un(s1,s2,{...})
%b
% - sistemato bug nella postproc di set term del tipo {.../cp(...)}
%   controesempio: diff({X,Y/cp(C,D)},cp(C,D),N)
%c
% - nuova implementazione (Maxi) di sat_eq_cp (caso cp(C,D)={...|cp(A,B)})
% - nuova implementazione (Maxi) di cp_to_set_u (caso X = cp({y/Sa},{z/Sb}))
...
%ground case - GFR - TO BE DONE
%ris_term(RIS,ris(_ in Dom,_,_,_,_)), extract_vars(RIS,LRis),  extract_vars(Dom,LDom), remove_list(LDom,LRis,LL).
%if LL={} then implementazione efficiente di espandi
...
ground_elems(cp(A,B)) :- !,      
       ground(A), ground(B).
ground_elems(Ris) :-            
       is_term(Ris,ris(_ in Dom,_,_,_,_)),!,
       ground_elems(Dom).
...
sunifyXt_special(T,X,Y,[set(N)]) :-               % X = {...|ris(Z in X,...,Z)}  (simple RIS)
      ris_term(T,ris(CtrlExpr in Dom,_,_,P,_)), 
      CtrlExpr == P,!,
      tail(Dom,TT),
      samevar(X,TT),
      replace_tail(Y,N,NewY),
      occur_check(X,NewY),          
      X = NewY.   
sunifyXt_special(T,X,Y,[solved(X = Y,var(X),1,f)]) :-  % X = {...|ris(Z in X,...,W)}  (non-simple RIS)
      ris_term(T,ris(_CtrlExpr in Dom,_,_,_,_)),    
      tail(Dom,TT),
      samevar(X,TT).
...
%%%%%%%%%%%% RIS 
sat_neq_ris([T1 neq T2|R1],R2,R,F):-                 % ris neq ris
        ris_term(T1,ris(_ in Dom1,_,_,_,_)),
        ris_term(T2,ris(_ in Dom2,_,_,_,_)),!, 
        nonopen_intv(Dom1),nonopen_intv(Dom2),
        sat_neq_ris_cont([T1 neq T2|R1],R2,R,F).
sat_neq_ris([T1 neq T2|R1],R2,R,F):-                 % ris neq t (t not ris)
        ris_term(T1,ris(_ in Dom,_,_,_,_)),!,
        nonopen_intv(Dom),nonopen_intv(T2),
        sat_neq_ris_cont([T1 neq T2|R1],R2,R,F).
sat_neq_ris([T1 neq T2|R1],R2,R,F):-                 % t neq ris (t not ris)
        ris_term(T2,ris(_ in Dom,_,_,_,_)),!,
        nonopen_intv(Dom),nonopen_intv(T1),
        sat_neq_ris_cont([T1 neq T2|R1],R2,R,F).

sat_neq_ris_cont([T1 neq T2|R1],R2,c,F):-            % ris(...) neq t or t neq ris(...) (t any term, including var and RIS)
...
sat_in_ris([X in T1|R1],R2,c,F):-           % ris: X in ris{v,{...},...)   
        ...
        ;
         sat_in([X in ris(CtrlExpr in D,V,Fl,P,PP) |R1],R2,_,F)
        ).
...
sat_nin_ris([X nin I|R1],[X nin I|R2],Stop,F):- % ris: X nin ris{v,D,...) (var-ris D): irreducible
        ris_term(I,ris(_ in Dom,_,_,_,_)), var_ris(Dom),!,   
        sat_step(R1,R2,Stop,F).
sat_nin_ris([_X nin I|R1],R2,c,F):-         % ris: X nin ris{v,{},...) -> true
        ris_term(I,ris(_ in Dom,_,_,_,_)), is_empty(Dom),!, 
        sat_step(R1,R2,_,F).
...
%         var_ris_st(S1),                              %GFR inutile
         nonvar(S2), is_empty(S2),!,  
         sunify(S1,{},C1),append(C1,R1,R3),   %%%%%NEW
         sat_step(R3,R2,_,F). 
...
%        nonvar(S3), S3=_ with T1,            % release setlog495-21, 21a, 21b
%        sunify(S3,N with T1,C1), !,

%        nonvar(S3), S3=N with T1, C1=[],!,   % setlog495-21c >= release =< setlog496-0

         nonvar(S3), S3=_ with T1,!,          % release >= setlog496-1
...
%          R = [T1 nin N,T1 nin N1, set(N1),set(N), delay(un(N1,S2,N),false)|C20]     % TOPLAS paper con delay
%          R = [T1 nin N,T1 nin S2, set(N1),set(N), un(S2,N1,N)|C20]     % release setlog495-21, 21a, 21b
%          R = [T1 nin S2, set(N1),set(N), un(S2,N1,N)|C20]              % setlog495-21c >= release =< setlog496-0
           R = [T1 nin N1, set(N1),set(N), un(N1,S2,N)|C20]              % release >= setlog496-1
         ;
           sunify(S2,N1 with T1,C2),                             % (ii)
           append(C1,C2,C20),
%          R = [T1 nin N,T1 nin N1, set(N1),set(N), un(S1,N1,N)|C20]     % TOPLAS paper
%          R = [T1 nin N,T1 nin N1, set(N1),set(N), delay(un(S1,N1,N),false)|C20]     % TOPLAS paper con delay
%          R = [T1 nin N,T1 nin S1, set(N1),set(N), un(S1,N1,N)|C20]     % release setlog495-21, 21a, 21b
%          R = [T1 nin S1, set(N1),set(N), un(S1,N1,N)|C20]              % setlog495-21c >= release =< setlog496-0
           R = [T1 nin N1, set(N1),set(N), un(S1,N1,N)|C20]              % release >= setlog496-1
         ;
...
%          R = [T1 nin N,T1 nin N1,T1 nin N2, set(N1),set(N2),set(N), delay(un(N1,N2,N),false)|C20]  % TOPLAS paper con delay
%          R = [T1 nin N, set(N1),set(N2),set(N), un(N1,N2,N)|C20]       % release setlog495-21, 21a, 21b
%          R = [set(N1),set(N2),set(N), un(N1,N2,N)|C20]                 % setlog495-21c >= release =< setlog496-0
           R = [T1 nin N1,T1 nin N2, set(N1),set(N2),set(N), un(N1,N2,N)|C20]  % release >= setlog496-1
         ),                
         append(R,R1,R3), 
%        sat_step(R3,R2,_,F).    % setlog495-21c >= release =< setlog496-0
         sat_nin(R3,R2,_,F).
...
ris_to_set_k(X,N,K) :-
        ...
        (nonvar(CNS),!,
         ...
         mk_atomic_constraint(FlNew,FlNewD),     %FlNewD is F(d) (dealt with as a constraint) 
         mk_atomic_constraint(PPNew,PPNewD),

         (K = [FlNewD, PPNewD, N = ris(CtrlExpr in D,V,Fl,P,PP) with Z | CNS]
         ;
         negate(FlNew,NegFl),
         mk_atomic_constraint(NegFl,NegFlD),     %NegFlD is not F(d) (dealt with as a constraint) 
...
sat_disj_ris([disj(X,Y)|R1],[disj(X,Y)|R2],Stop,F):-         % disj(X,Y), var_ris(X), var_ris(Y) (irreducible form)
        ...
        sat_step(R1,R2,Stop,F).
sat_disj_ris([disj(X,Y)|R1],R2,c,F):-                        % disj(X,ris(...)) --> disj(ris(...),X)  [rule-ris ||_4 or ||_6]
        ris_term(Y), 
        \+ris_term(X),!,
        sat_disj_ris([disj(Y,X)|R1],R2,_,F).   
sat_disj_ris([disj(Ris,Set)|R1],R2,c,F):-                    % disj(ris(...),{...})    [rule-ris ||_5]
        nonvar(Set), Set = T2 with T1,!,
...
sat_disj_ris([disj(I,A)|R1],R2,c,F):-                        % disj(ris(...),A), var(A) or ris(A)  [rule-ris ||_7]
       ...
       ;
        sat_step_ris([disj(ris(CtrlExpr in D,V,Fl,P,PP),A) |R1],R2,_,F)
       ).
...
ris_preproc(RisE,ris(New_CE_Dom,LVars,New_Fl,New_P,New_PP),C) :-      
        is_ris(RisE,ris(CE_Dom,LVars,Fl,P,PP)),
        preproc(CE_Dom,New_CE_Dom,C1),
        preproc(Fl,New_Fl,C2),
        preproc(P,New_P,C3),
        preproc(PP,New_PP,C4),
        check_ris(ris(New_CE_Dom,LVars,New_Fl,New_P,New_PP)),!,
        append(C1,C2,C12), append(C12,C3,C123), append(C123,C4,C).
ris_preproc(_,_,_):- 
        msg_sort_error(ris), 
        fail.
...
replace_tail(R,N,ris(X in Dnew,V,F,P,PP)) :- 
       ris_term(R,ris(X in D,V,F,P,PP)),!, 
...
type_constraints_to_be_shown([set(_),bag(_),list(_),integer(_),rel(_),pfun(_),pair(_),   
                              nset(_),ninteger(_),npfun(_),npair(_)]).     %all  
